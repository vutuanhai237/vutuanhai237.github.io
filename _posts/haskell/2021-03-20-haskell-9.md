...
Trong phần trước, tôi đã trình bày những điều cơ bản về class. Tôi có giải thích rằng class là interface định nghĩa một hành vi nào đó. Một kiểu dữ liệu có thể kế thừa class nếu như class này định nghĩa hành vi của kiểu dữ liệu. Lấy ví dụ: kiểu Int là kế thừa của class Eq vì class Eq có định nghĩa hành vi so sánh ngang bằng được. Và vì số nguyên có thể so sánh ngang bằng nên Int là thuộc class Eq. Lợi ích thực sự đến từ các hàm tạo nên interface cho Eq, cụ thể là == và /=. Nếu một kiểu dữ liệu thuộc class Eq, ta có thể dùng hàm ==. Điều này lý giải tại sao những biểu thức như 4 == 4 và "foo" /= "bar" lại kiểm tra về kiểu.

Tôi cũng nhắc lại rằng class chứa kiểu thường hay bị nhầm với class trong OOP, vốn từng làm rối nhiều người. Ở OOP, class là "bí kíp" để tạo ra các đối tượng chứa trạng thái, có thể kèm theo hành động như bản năng. Class chứa kiểu thì giống interface hơn. Ta không thể tạo ra dữ liệu từ class chứa kiểu (để cho gọn tôi gọi chúng là class). Mà trước hết, ta tạo ra kiểu dữ liệu rồi sau đó nghĩ xem nó có thể hoạt động thế nào. Nếu nó so sánh ngang bằng được, ta sẽ cho kế thừa từ class Eq. Nếu nó có thể sắp xếp, ta sẽ cho kế thừa từ class Ord.
Trong mục kế tiếp, ta sẽ xem cách tạo ra chính các class, bằng cách code những hàm định nghĩa class. Nhưng ngay bây giờ, hãy xem Haskell có thể tự động tạo ra kiểu nào trong danh sách sau đây: Eq, Ord, Enum, Bounded, Show, Read. 
...
```
Kiểu dữ liệu này mô tả một người. Giả định rằng không có hai người nào cùng chung tổ hợp họ, tên, và tuổi. Bây giờ, hãy thử so sánh ngang bằng xem chúng có bằng nhau không. Điều này lý giải tại sao kiểu trên lại thuộc về class Eq. Ta sử dụng kế thừa như sau.
```
data Person = Person { 
    firstName :: String,
    lastName :: String, 
...
True
```
Dĩ nhiên, vì `Person` hiện giờ trong Eq, ta có thể dùng nó như là `a` với mọi hàm có ràng buộc `class Eq a` trong type signature của chúng, chẳng hạn như elem.
```
ghci>:t elem
...
True
```
Class Show và Read lần lượt dùng cho những kiểu có thể chuyển thành chuỗi hoặc ngược lại. Cũng như Eq, nếu một kiểu có constructor chứa các trường, thì các trường này phải thuộc Show hoặc Read nếu ta muốn kiểu của ta kế thừa các class đó. 
```
data Person = Person { 
    firstName :: String,
    lastName :: String, 
...
Ta có thể kế thừa class Ord. Nếu ta so sánh hai giá trị cùng một kiểu nhưng được tạo thành từ các constructor khác nhau thì giá trị được tạo bởi constructor định nghĩa trước là nhỏ hơn.
...
True
```
Lưu ý rằng chúng ta không thể so sánh kiểu `Just (*3) > Just (*2)`, vì `(*3)` và `(*2)` là hàm, và không được kế thừa class Ord.

![image info](https://github.com/vutuanhai237/vutuanhai237.github.io/blob/master/assets/image/haskell/9-5.png?raw=true)

Ta có thể dùng kiểu dữ liệu đại số để thực hiện việc liệt kê, class Enum và Bounded sẽ giúp ta thực hiện điều đó. Xét kiểu dữ liệu sau đây:
```
data Day = Monday | Tuesday | Wednesday | Thursday | Friday | Saturday | Sunday
```
Vì tất cả những constructor value đều không nhận tham số (tức là trường) nên ta có thể cho nó thuộc về class Enum. Class Enum dành cho những thứ có các giá trị liền trước và liền sau. Ta cũng có thể cho nó thuộc về class Bounded (kiểu có biên trên và biên dưới). Xong việc.
```
data Day = Monday | Tuesday | Wednesday | Thursday | Friday | Saturday | Sunday deriving (Eq, Ord, Show, Read, Bounded, Enum)
```
Vì kiểu này nằm trong class Show và Read nên ta có thể chuyển những giá trị thuộc kiểu này sang String và ngược lại.
```
ghci> Wednesday
Wednesday
ghci> show Wednesday
...
Saturday
```
Vì `Day` nằm trong class Eq và Ord, nên ta có thể so sánh hơn kém hoặc ngang bằng các ngày với nhau.
```
...
LT
```
Nó nằm trong class Bounded, vì vậy ta có thể lấy ngày "thấp nhất" và "cao nhất".
```
ghci> minBound :: Day
Monday
ghci> maxBound :: Day
