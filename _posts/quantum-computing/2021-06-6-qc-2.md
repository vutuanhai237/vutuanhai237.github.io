...
Chiến thuật tìm các điểm cực tiểu được mô tả sau. Chúng ta có hàm $f$ phụ thuộc biến $x\in(x^*\cup X)$ với $x^*$ là cực tiểu toàn cục và $X=\{x^{(0)},x^{(1)},...,x^{(n)}\}$ là tập các thử nghiệm (ansatz) trong $n$ vòng lặp, $x^{(0)}$ là giá trị khởi tạo, $x$ không chỉ là giá trị vô hướng mà có thể là vector, ma trận hoặc đại lượng nhiều chiều hơn. Ở đây ta cần đưa $x\approx x^*$ càng tốt và $x=x^*$ là tốt nhất.
...
#### **2. Parameter shift**
Giả sử ta có chuỗi các cổng chứa tham số $U(\bm{\theta})$ phụ thuộc vào $\bm{\theta}$ và quan sát $\hat{B}$, phụ thuộc vào quan sát có nghĩa là, chẳng hạn, nếu $\hat{B}$ là ma trận Pauli $$Z=\begin{bmatrix}1 & 0\\0 & -1\end{bmatrix}.$$Khi đo được qubit trong trạng thái $\mid 0\rangle$ hoặc $\mid 1\rangle$ thì ta có thể suy ra giá trị cần tìm là $1$ hoặc -$1$ tương ứng là trị riêng của pauli $Z$.
...
Hấp thụ $V$ vào đại lượng quan sát $\hat{B}$ và $W$ vào trạng thái $|0\rangle$, ta thể $V^\dagger\hat{B}V=\hat{Q}$, $W|0\rangle=\mid\psi\rangle$ và $\langle 0 \mid W=\langle\psi\mid$ vào phương trình (3.1):
...
Trường hợp 2: nếu $G=r\overrightarrow{n}\bm{\sigma}$ là tổ hợp tuyến tính của các toán tử Pauli $\bm{\sigma}$ với $\overrightarrow{n}$ là vector 3 chiều bình thường.
