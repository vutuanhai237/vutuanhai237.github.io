...
Chiến thuật tìm các điểm cực tiểu được mô tả sau. Chúng ta có hàm $f$ phụ thuộc biến $x\in\;(x^{\ast}\cup\;X)$ với $x^{\ast}$ là cực tiểu toàn cục và $X=\{x^{(0)},x^{(1)},...,x^{(n)}\}$ là tập các thử nghiệm (ansatz) trong $n$ vòng lặp, $x^{(0)}$ là giá trị khởi tạo, $x$ không chỉ là giá trị vô hướng mà có thể là vector, ma trận hoặc đại lượng nhiều chiều hơn. Ở đây ta cần đưa $x\approx\;x^{\ast}$ càng tốt và $x=x^{\ast}$ là tốt nhất.
...
Hấp thụ $V$ vào đại lượng quan sát $\hat{B}$ và $W$ vào trạng thái $\mid0\rangle$, ta thể $V^\dagger\hat{B}V=\hat{Q}$, $W\mid\;0\rangle=\mid\psi\rangle$ và $\langle 0 \mid W=\langle\psi\mid$ vào phương trình (3.1):
...
Chứng minh:
Với $f=e^{-i\mu\frac{\sigma_i}{2}}$, $\partial_\mu f=\frac{(-i\sigma_i)}{2}f=(-\frac{1}{2}if)\sigma_i$.

Theo phương trình (3.6), vì $\frac{1}{2}\sigma_i$ có trị riêng là $\pm\frac{1}{2}$ nên $r=1/2$ và $s=\frac{\pi}{4r}=\frac{\pi}{2}$.
...
$
=\frac{i}{2}\langle\psi\mid\mathcal{G}^\dagger[\sigma_i,\hat{Q}]\mathcal{G}\mid\psi\rangle\;(4.1)
$

Với $[\sigma_i,\hat{Q}]=-i(\mathcal{G}^\dagger(\frac{\pi}{2})\hat{Q}\mathcal{G}(\frac{\pi}{2})-\mathcal{G}^\dagger(-\frac{\pi}{2})\hat{Q}\mathcal{G}(-\frac{\pi}{2}))$ trong [Mitarai et al. (2018)](https://arxiv.org/pdf/1803.00745.pdf), (4.1) trở thành:
...
$
=\frac{1}{2}(f(\mu+\frac{\pi}{2})-f(\mu-\frac{\pi}{2}))
$
...
Nhìn chung, **parametet shift** sẽ có một vài điểm vượt trội hơn phương pháp cũ như sau:
...
(3) Độ dời (shift) ở đây được xác định, không phải là vô cùng bé như phương pháp sai phân hữu hạn do đó chúng ta có thể cực đại hóa khoảng cách giữa hai lần cập nhật trong không gian tham số. Vấn đề hội tụ được đảm bảo.
